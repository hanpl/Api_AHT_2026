...
{
    public class EntranceCRepository
    {
        string connectionString;
        public EntranceCRepository(string connectionString)
        {
            this.connectionString = connectionString;
        }
        #region Get Entrance A
        public List<AHT_EntranceC> GetEntrance()
        {
            List<AHT_EntranceC> aHT_Entrances = new List<AHT_EntranceC>();
            AHT_EntranceC aHT_Entrance;
            ...
            foreach (DataRow row in data.Rows)
            {
                ...
                aHT_Entrance = new AHT_EntranceC()
                {
        ...
        public DataTable GetCodeShareById(string id)
        {
            ...
            "where TRY_CONVERT(datetime, A.Mcdt,    103) between DATEADD(Mi, -50, getdate()) and DATEADD(hh, 6,getdate()) AND A.Status<>'' and A.Status<> 'Cancelled' " +
            "and A.Adi = 'D' AND RowFrom NOT LIKE 'M%' AND ISNUMERIC(RowFrom) = 1 and A.RowFrom <= 27 and A.Id = '" + id + "' Order by A.Mcdt ASC"; ;
        ...
        {
            string query = "SELECT LineCode, CONCAT(LineCode, Number) AS Flight, Mcdt, Schedule,Estimated, RowFrom, RowTo, Id FROM AHT_FlightInformation WHERE TRY_CONVERT(datetime, Mcdt,    103) BETWEEN DATEADD(Mi, -50, GETDATE()) AND DATEADD(hh, 6, GETDATE()) " +
              " AND Status<> '' " +
        ...
        public List<AHT_EntranceC> GetEntranceB()
        {
            List<AHT_EntranceC> aHT_Entrances = new List<AHT_EntranceC>();
            AHT_EntranceC aHT_Entrance;
        ...
        {
            string query = "SELECT LineCode,CONCAT(LineCode, Number) AS Flight, Mcdt, Schedule, Estimated, RowFrom, RowTo, Id FROM AHT_FlightInformation WHERE TRY_CONVERT(datetime, Mcdt,    103) BETWEEN DATEADD(Mi, -20, GETDATE()) AND DATEADD(hh, 6, GETDATE()) " +
              " AND Status<> '' " +
              " AND Status<> 'Cancelled' " +
              " AND Adi = 'D' " +
              " AND RowFrom NOT LIKE 'M%' " +
